...
class Task(BaseModel):
    ...
    updated_part: float = Field(alias="updPart")  # Part of the task completed at last update
    amount_multiplier: int = Field(alias="mul")  # Amount multiplier (both input and output, level 2 building produces 2x)
...
class Workforce(BaseModel):
    ...
    workers_needed: List[int] = Field(alias="workersNeeded")  # Quantity of each type: [Worker, Technician, Engineer, Scientist]
    workers_housing: List[int] = Field(alias="workersHousing")  # Housing capacity by type
    workers_count: List[int] = Field(alias="workersCount")  # Current number of each type of worker
    consumption_materials: List[ConsumptionMaterial] = Field(alias="consumptionMaterials")
    workers_satisfaction: List[float] = Field(alias="workersSatisfaction")  # Satisfaction levels for each type of worker
...
class Warehouse(BaseModel):
    ...
    materials: List[Material] = Field(alias="mats")  # List of materials stored in the warehouse
class Base(BaseModel):
    ...
    production_orders: List[ProductionOrder] = Field(alias="productionOrders", default=[])
